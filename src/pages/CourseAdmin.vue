        ...
        <el-dialog title="新增课程" :visible.sync="openCourse" width="50%" >
            <div style="overflow:auto;">
                <div>
                    ...
                    <el-col :span="18">
                        <el-input size="mini" v-model="inputName" placeholder="请输入课程名"></el-input>
                    </el-col>
                </div>
            </div>
...
export default {
    ...
    methods:{
        ...
        additionCourse(){
            //发送请求增加课程
            axios({
                url:this.$URL.mqttUrl+"/score/course/add/"+`${this.inputName}`,
                method:"POST"
            }).then((response)=>{
        ...
        del(row){
            //发送请求删除课程
            axios({
                url:this.$URL.mqttUrl+"/score/course/del/"+`${row.courseid}`,
                method:"delete"
            }).then((response)=>{
                if(response.data.code == 200){
                    this.$message({
        ...
        searchCourseName(){
            ...
            if(this.searchInput!=''){
                axios({
                    url:this.$URL.mqttUrl+"/score/course/info/search/"+`${this.searchInput}`,
                    method:"get"
                }).then((response)=>{
